...
SELECT last_name, COUNT(last_name) AS "Number of actors with same last name"
FROM actor
GROUP BY last_name;
...
SELECT f.title, COUNT(i.inventory_id) AS "Number of copies in Inventory"
FROM film f
...
SELECT sto.store_id, SUM(amount) AS "Revenue" 
FROM store sto
...
SELECT c.name, SUM(p.amount) AS "Revenue per Genre" 
FROM category c
